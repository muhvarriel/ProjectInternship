	...
	<section>
		<nav class="col-12 navbar navbar-expand-sm bg-dark navbar-dark">
		  <ul class="navbar-nav">
		    ...
		    <li class="nav-item active">
		      <a class="nav-link" href="?p=dosenhome">Home</a>
		    </li>
		    <li class="nav-item">
		      <a class="nav-link" href="?p=profiledosen&id=<?php echo $_SESSION["id"];?>">Account</a>
		    </li>
		  </ul>
		...
		<article class="col-10 student thin">
			...
			<table id="myTable">
		  	...
		  	<?php
		  		...
		  		$arrayResult = $objInternship->SelectInternshipAccept($idMajor);
		  		...
		  		if(count($arrayResult) == 0){
		  			...
		  			foreach ($arrayResult as $dataInternship) {
		  				echo '<tr>';
		  					...
		  					echo '<td><a class="btn btn-success btn-sm"  href="?p=dosenguide&idInternship='.$dataInternship->idInternship.'">View</a>
		  							  </td>';					
		  				echo '</tr>';				
		  				$no++;	
		  			}
